...
class INA260:
    ...
    def __init__(self, i2c_bus: I2C, address: int = 0x40, ina2xx_id: int = None, current_factor: float = 1.25) -> None:
        ...
        if device_id != self.INA260_ID:
            if ina2xx_id is None:
                raise RuntimeError(
                    f"Failed to find INA260 ID, read {device_id} while expected {self.INA260_ID}"
                    " - check your wiring!"
                )
        elif device_id == ina2xx_id:
                print(
                    f"Found the expected devide with ID {ina2xx_id}"
                )
        else:
                print(
                    f"Failed to found device with expected ID {ina2xx_id} and found instead a device with ID {device_id}"
                )
    ...
    def current(self) -> float:
        ...
        if self.mode == Mode.TRIGGERED:
            while self._conversion_ready_flag == 0:
                pass
        return self._raw_current * self._current_factor
    ...
    def voltage(self) -> float:
        ...
        if self.mode == Mode.TRIGGERED:
            while self._conversion_ready_flag == 0:
                pass
        return self._raw_voltage * (self._current_factor / 1000.0)
